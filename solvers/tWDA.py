...
class tWDA(BaseEstimator, ClassifierMixin, TransformerMixin):
    ...
    def __init__(self,n,dfs,n_jobs=1):
        """Init."""
    ...
    def fit(self, S, y):
        ...
        if len(self.dfs)==1:
            self.dfs=[self.dfs[0] for _ in range(Nc)]
        else:
            if len(self.dfs)!=Nc:
                raise NameError('The list of dof invalid, must contain one element or have the same length as the number of classes')

        y = np.asarray(y)
        p,_ = S[0].shape
        if self.n_jobs==1:
            self.centers = [self.compute_class_center(S[y==self.classes_[i]],self.dfs[i]) for i in range(Nc)]
        else:
            self.centers = Parallel(n_jobs=self.n_jobs)(delayed(self.compute_class_center)(S[y==self.classes_[i]],self.dfs[i]) for i in range(Nc))
    ...
    def _predict_distances(self, covtest):
        ...
        for i in range(Nc):
            if self.dfs[i]==np.inf:
                log_h = lambda t:-0.5*t
            else:
                log_h = lambda t:-0.5*(self.dfs[i]+self.n*p)*np.log(1+t/self.dfs[i])
